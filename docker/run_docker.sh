...
if [ $? -eq 0 ]; then
    ...
    docker-compose -f docker/docker-compose.yml logs --tail=10 api
    
    echo -e "\n${GREEN}To run tests with real models:${NC}"
    echo -e "  docker-compose -f docker/docker-compose.yml run --rm improvements python -m src.run_improvements --models \"distilgpt2\" --max-samples 5"
    
    # Show OpenRouter commands if enabled
    if [ "$OPENROUTER" = true ] || [ "$SERVICES" = "openrouter-api openrouter-cli" ]; then
        echo -e "\n${GREEN}To run OpenRouter CLI:${NC}"
        echo -e "  docker-compose -f docker/docker-compose.yml run --rm openrouter-cli python -m src.menu"
        
        echo -e "\n${GREEN}To test a model with OpenRouter:${NC}"
        echo -e "  docker-compose -f docker/docker-compose.yml run --rm openrouter-cli python -m src.menu --model mistral-7b --prompt \"Your prompt here\""
        
        echo -e "\n${GREEN}To run bias tests with OpenRouter:${NC}"
        echo -e "  docker-compose -f docker/docker-compose.yml run --rm openrouter-cli python -m src.menu --model mistral-7b --bias-test"
    fi
